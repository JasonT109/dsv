...
public class WindowManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        // Check if an explicit screen position is specified in commandline.
        var hasXPos = CommandLine.HasParameter("screen-position-x");
        var hasYPos = CommandLine.HasParameter("screen-position-y");
        if (!hasXPos && !hasYPos)
            return;

        // If so, move window to the desired position
        var x = CommandLine.GetParameter("screen-position-x", 0);
        var y = CommandLine.GetParameter("screen-position-y", 0);
        SetPosition(x, y);
    }
