...
Shader "Shader Forge/mapTerrain" {
    Properties {
        ...
        _LineDetail ("LineDetail", Range(0, 1)) = 1
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
/////// Diffuse:
                ...
                float node_936 = 0.5;
                float3 node_2938 = (lerp(_LineColor.rgb,_LineColorHi.rgb,(1.0 - node_5353))*((1.0 - (smoothstep( node_5683, node_746, frac((node_4040*_Levels)) )*smoothstep( node_5683, node_746, frac((node_4040*(_Levels*(-1.0)))) )))*((step(node_1762,node_8168)+node_936)*(step((_Separation-0.1),node_8168)+node_936))*((1.0 - (smoothstep( node_5683, node_746, frac(node_4040) )*(1.0 - _LineDetail)))*1.0)));
